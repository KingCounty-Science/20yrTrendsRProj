...
calibrate.bins <-
function(bin.mid,N.avail,N.use,plot=TRUE,...){

  #------------------------------------------------------------------------------------------------#
  # This function calculates the coefficient of concordance of a set of predictions 
  # based on presense and psuedo-absense data in binned (tabular) form 
  # for the purpose of model calibration; i.e., determining the accuracy of the 
  # predicted probability of presence when applied to the entire study area.
  #
  # Kevin McGarigal July 17, 2017
  #
  # Arguments:  bin.mid : vector of bin mid-point predicted values (must be in the range 0,1)
  #             N.avail : vector of available count in each bin
  #             N.use : vector of use count in each bin
  #             plot : logical should a plot be made
  #             ... : optional other arguments passed to plot
  #------------------------------------------------------------------------------------------------#  
  ...
  stopifnot(all(bin.mid>=0 & bin.mid<=1))
  
  #create objects
  bin.number<-seq(1,length(bin.mid))
 
  #warning condition check
  if(any(avail==0 & use!=0)) 
    warning(paste("Values were observed for bins where there were no available; ", 
               "i.e., use count > 0 and available count = 0. This can't happen",
               "with a census, but it can with a sample. Ideally, all bins should",
               "have available counts > 0. You may want to conduct a census or",
               "increase your available sample or reduce the number of bins",
               "until the available counts are all > 0."))
  
  #remove bins where there are no use or available counts
  bin.number2<-bin.number[!(N.avail==0 & N.use==0)]
  bin.mid2<-bin.mid[!(N.avail==0 & N.use==0)]
  N.avail2<-N.avail[!(N.avail==0 & N.use==0)]
  N.use2<-N.use[!(N.avail==0 & N.use==0)]
  P.use2<-N.use2/sum(N.use2)
  
  #compute expected utilization (proportion) and counts by bin
  denom<-sum(bin.mid2*N.avail2)
  P.exp2<-(bin.mid2*N.avail2)/denom
  N.exp2<-P.exp2*sum(N.use2)

  #compute coefficient of concordance
  #cc<-epi.ccc(N.exp,N.obs,ci="z-transform",conf.level=0.95)$rho.c
  cc<-round(epi.ccc(P.exp2,P.use2,ci="z-transform",conf.level=0.95)$rho.c,2)

  #create result object
  result<-list(bin.counts=data.frame(bin=bin.number2,bin.mid=bin.mid2,
      N.avail=N.avail2,P.exp=P.exp2,N.exp=N.exp2,N.use=N.use2,
      P.use=P.use2),concordance=cc)

  #plot observed vs expected proportions
  if(plot){
    plot(P.exp2,P.use2,xlab='Expected proportion',ylab='Observed proportion',...)
    abline(0,1,lty=2,lwd=2)
    mtext(text=paste('Coefficient of Concordance = ',cc[1],sep=''),side=3,cex=1.5)
    }
...
ci.lines <-
function(model,interval=95,type='expected',...){
xm<-mean(model[[12]][,2])
n<-length(model[[12]][,2])
ssx<-sum(model[[12]][,2]^2)-sum(model[[12]][,2])^2/n
prob<-1-((1-(interval/100))/2)
s.t<-qt(prob,(n-2))
xv<-seq(min(model[[12]][,2]),max(model[[12]][,2]),(max(model[[12]][,2])-min(model[[12]][,2]))/100)
yv<-coef(model)[1]+coef(model)[2]*xv
if(type=='expected') se<-sqrt(summary(model)[[6]]^2*(1/n+((xv-xm)^2/ssx)))
else if(type=='predicted') se<-sqrt(summary(model)[[6]]^2*(1+(1/n)+((xv-xm)^2/ssx)))
else stop('Invalid interval specified, must be expected or prediction')
ci<-s.t*se
uyv<-yv+ci
lyv<-yv-ci
lines(xv,uyv,...)
...
G<-length(levels(grp))
z<-matrix(NA,perm,G+2) #create blank matrix

for(i in 1:perm){
	out<-tryCatch(
  {
  ran.split(y,grp,prop=prop)
	if(type=='qda'){
      if(missing(prior)) y.da<-qda(training.data,grouping=training.grp)
      else y.da<-qda(training.data,grouping=training.grp,prior=prior)
	} else if(type=='lda'){
	  if(missing(prior)) y.da<-lda(training.data,grouping=training.grp)
	  else y.da<-lda(training.data,grouping=training.grp,prior=prior)
  }
	},
  error=function(e) e
  )
  ...
  #create confusion matrix and compute correct classification rates
...
#summarize results across permutations
cat('Number of valid random training data sets')
print(sum(!is.na(z[,1])))
...
function(x,grp,digits=3){
  ...
  covar<<-function(x,na.rm) sd(x,na.rm=TRUE)/mean(x,na.rm=TRUE)*100 
  x<-as.data.frame(x)
  grp<-as.factor(grp)
  groups<-levels(grp)
  ...
  cl.cv<-aggregate(x,by=list(grp),covar)
  cl.cv<-cl.cv[,-1]
...
if(method=='correlation'){ #compute correlation distance
  ...
  for(i in 1:nrow(y)){
    for(j in 1:nrow(y)){
      vcov<-cov(y)
      z[i,j]<-mahalanobis(as.numeric(y[i,]),center=as.numeric(y[j,]),cov=vcov)
    }
  }
  z<-as.dist(sqrt(z))  
  if(!outfile==''){ #save outfile
...
data.trans <-
function(x,method,var='',exp=1,ref=0,outfile='',
...
else if(method=='logit'){ #logit transformation
  
  #verify proportional data
  if(min(t1,na.rm=TRUE)<0 | max(t1,na.rm=TRUE)>1) stop('logit transformation not valid for non-proportional data')
  
  else{
    t1<-log(t1/(1-t1)) #compute logit transformation
    t1<-as.data.frame(round(t1,3)) #round to 3 decimal places
    if(plot==TRUE){
      paired.hist(y1,t1)
    }
...
else if(method=='asin'){ #arcsine squareroot transformation

  #verify proportional data
  if(min(t1,na.rm=TRUE)<0 | max(t1,na.rm=TRUE)>1) stop('arcsine squareroot transformation not valid for non-proportional data')
...
else if(method=='cosine'){ #cosine transformation
  
  #verify data is in degrees
  if(min(t1,na.rm=TRUE)<0 | max(t1,na.rm=TRUE)>360){
    stop('cosine transformation not valid for non-circular data; requires data given in degrees')
  }
  
  else{

    #create degree to radian function
    deg2rad<-function(t1,ref) {((ref-t1)*pi)/(180)}
    t1<-deg2rad(t1,ref)
    t1<-(cos(t1)+1)/2 #compute cosine transformation
    t1<-as.data.frame(round(t1,3)) #round to 3 decimal places
    if(plot==TRUE){
      paired.hist(y1,t1)
    }
...
ordi.part <-
function(y,x1,x2,x3=NULL,p=NULL,method='rda',model='reduced',digits=3,...){
...
##permutation tests
if(is.null(x3)){
	x1.pv<-anova(z1,model=model,...)['Model','Pr(>F)']
	x2.pv<-anova(z2,model=model,...)['Model','Pr(>F)']
	v1.pv<-anova(z1.2,model=model,...)['Model','Pr(>F)']
	v2.pv<-anova(z2.1,model=model,...)['Model','Pr(>F)']
	}
else{
	x1.pv<-anova(z1,model=model,...)['Model','Pr(>F)']
	x2.pv<-anova(z2,model=model,...)['Model','Pr(>F)']
	x3.pv<-anova(z3,model=model,...)['Model','Pr(>F)']
	x12.pv<-anova(z12,model=model,...)['Model','Pr(>F)']
	x13.pv<-anova(z13,model=model,...)['Model','Pr(>F)']
	x23.pv<-anova(z23,model=model,...)['Model','Pr(>F)']
	v1.pv<-anova(z1.23,model=model,...)['Model','Pr(>F)']
	v2.pv<-anova(z2.13,model=model,...)['Model','Pr(>F)']
	v3.pv<-anova(z3.12,model=model,...)['Model','Pr(>F)']
	}
...
if(is.null(x3)){
	...
	#other components
	if(!is.null(p)) out$conditional<-c(z12$pCCA$tot.chi,z12$pCCA$tot.chi/z12$tot.chi)
	...
	cat('\nComponents:','\n')
	    ...
	    print(round(out$components,digits=digits))
	#other components
	if(!is.null(p)) out$conditional<-c(z123$pCCA$tot.chi,z123$pCCA$tot.chi/z123$tot.chi)
